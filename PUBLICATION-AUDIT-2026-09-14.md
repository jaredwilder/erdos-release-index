...
### Fiber coherence — corrected theorem headline

The top-level [`fiber-coherence-cycle-rank`](https://github.com/jaredwilder/fiber-coherence-cycle-rank) README previously led with the four-type cyclomatic-rank-three kernel classification. The final court identifies that topology as useful but classical-adjacent infrastructure.

The front door now leads with the program's more distinctive source theorem pair:

- every finite binary relation has an exact finite **K4-free fiber strip** realizing precisely its allowed boundary pairs;
- composing those strips gives a **polynomial-size exact K4-free fiber-coherence realization of every finite binary CSP**.

The formulation-specific NP-completeness consequence and the fixed-rank/unbounded-rank boundary remain scope-separated. Historical novelty remains an `N2` candidate requiring specialist graph-gadget/CSP/homomorphism review; Erdős #595 remains open.

### R(5,5) order-41 circulant phase transition

The top-level [`ramsey-r55-circulant-41`](https://github.com/jaredwilder/ramsey-r55-circulant-41) README now exposes the actual structural endpoint rather than merely listing ingredients.

The public program records that the 20 labelled half-density circulant `(5,5)` witnesses on `Z_41` form one affine-isomorphism class. For that unique class, one arbitrary new vertex produces a 41-variable / 2050-clause extension CNF; independent mixed-integer and DPLL implementations find it infeasible, with a 21-node normalized DPLL tree.

Thus no order-41 circulant `(5,5)` Ramsey graph can remain intact as an induced 41-vertex core of an order-42 `(5,5)` Ramsey graph. This is a structural nonextension result, **not** a new Ramsey-number bound and not a claim that the classical order-41 graph is new. Historical novelty of the structural refinements remains unresolved.
...
### Removed false debt: AGI-ZETA-BOUNDED-CLOSE-2026-08-14

An earlier audit treated `AGI-ZETA-BOUNDED-CLOSE-2026-08-14.zip` as a pointer to missing RH mathematics. The later raw-ore industrial ingest explicitly corrected that classification after Library inspection: `AGI-ζ` was an unrelated bounded cross-domain learning packet using a Greek-letter project naming sequence, **not** a missing Riemann-zeta packet.

It is therefore removed from the RH recovery debt. This corrects the classification; it does not claim recovery or validation of a zeta theorem.
