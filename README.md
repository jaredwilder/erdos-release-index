...
| [graham-alspach-z53-z71](https://github.com/jaredwilder/graham-alspach-z53-z71) | **88 sequenceability rows at size 21 or above**, against a published general result covering size at most 20. **14,530,824,658 orbit representatives covering 603,776,962,733 subsets, zero non-sequenceable.** `Z_53` (sizes 41-52) and `Z_71` (62-70) had never been attempted; `Z_37` and `Z_41` are now complete from 21 to `p-1`. Every row's orbit sizes sum to exactly `C(p-1,c)`. The compute wall is stated exactly, and a hand-checkable `Z_53` witness is included. |
